 ...
 * The whole idea behind the inline functions is that whenever inline 
 * function is called code of inline function gets inserted or substituted
 * at the point of inline function call at compile time. Inline functions 
 * are very useful when small functions are frequently used and called in 
 ...
 * inline is a request to the compiler and its compilers choice to do 
 * inlining or not).
 ...
 * called using the object (without reference or pointer) of that class, can
 * be inlined because compiler knows the exact class of the object at 
 * compile time.
 */
